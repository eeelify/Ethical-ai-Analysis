    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr),380px]">
        ...
        <aside className="rounded-lg border border-white/10 bg-[#050b14] p-4">
          <h4 className="mb-3 text-sm font-semibold text-white">Assessment report</h4>
          {!userReport ? (
            ...
            <div className="max-h-[780px] space-y-3 overflow-y-auto pr-1">
              <section className="rounded-lg border border-cyan-500/20 bg-cyan-500/10 p-4">
                <h3 className="text-base font-semibold text-white">{userReport.title}</h3>
                <div className="mt-3 inline-flex rounded-full border border-cyan-400/30 bg-[#050b14] px-3 py-1 text-xs font-medium text-cyan-200">
                  Overall assessment: {userReport.overallAssessment}
                </div>
                <p className="mt-3 text-sm leading-6 text-slate-300">{userReport.overallExplanation}</p>
              </section>

              <ReportSection title="Executive summary">
                <p className="text-sm leading-6 text-slate-300">{userReport.executiveSummary}</p>
              </ReportSection>

              <ReportSection title="System overview">
                <div className="space-y-2">
                  {userReport.overview.map((item) => (
                    <div key={item.label} className="grid gap-1 text-sm md:grid-cols-[130px,1fr]">
                      <div className="text-slate-500">{item.label}</div>
                      <div className="text-slate-300"><InlineList value={item.value} /></div>
                    </div>
                  ))}
                </div>
              </ReportSection>
              ...
              <ReportSection title="Main risks">
                {userReport.risks.length === 0 ? (
                  <p className="text-sm text-slate-400">No applicable risks have been identified from the current facts yet.</p>
                ) : (
                  <div className="space-y-3">
                    {userReport.risks.map((risk) => (
                      <div key={risk.title} className="rounded-lg border border-white/10 bg-[#050b14] p-3">
                        ...
                        <p className="mt-2 text-sm leading-6 text-slate-300">{risk.reason}</p>
                        <p className="mt-2 text-xs leading-5 text-slate-400"><span className="font-medium text-slate-300">Possible impact:</span> {risk.impact}</p>
                        {risk.existingSafeguard ? (
                          <p className="mt-1 text-xs leading-5 text-slate-400"><span className="font-medium text-slate-300">Existing safeguard:</span> {risk.existingSafeguard}</p>
                        ) : null}
                      </div>
                    ))}
                  </div>
              ...
              <ReportSection title="Confirmed safeguards">
                {userReport.safeguards.length === 0 ? (
                  <p className="text-sm text-slate-400">No safeguards have been confirmed yet.</p>
                ) : (
                  <ul className="space-y-2 text-sm text-slate-300">
                    {userReport.safeguards.map((item) => (
              ...
              <ReportSection title="Information still needed">
                {userReport.missingQuestions.length === 0 ? (
                  <p className="text-sm text-slate-400">No high-priority missing information is currently shown.</p>
                ) : (
                  <ol className="list-decimal space-y-2 pl-5 text-sm leading-6 text-slate-300">
                    {userReport.missingQuestions.map((item) => (
                      <li key={item}>{item}</li>
                    ))}
                  </ol>
                )}
              ...
              <ReportSection title="Regulatory considerations">
                {userReport.regulatory.length === 0 ? (
                  <p className="text-sm text-slate-400">No regulatory area has enough support from the current facts yet.</p>
                ) : (
                  <div className="space-y-3">
                    {userReport.regulatory.map((item) => (
                      <div key={item.area} className="rounded-lg border border-white/10 bg-[#050b14] p-3">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <h5 className="text-sm font-semibold text-white">{item.area}</h5>
                        ...
                        <p className="mt-1 text-xs leading-5 text-slate-400"><span className="font-medium text-slate-300">Additional information required:</span> {item.additionalInformation}</p>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <ReportSection title="Recommended actions">
                <ol className="list-decimal space-y-2 pl-5 text-sm leading-6 text-slate-300">
                  {userReport.actions.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ol>
              </ReportSection>

              {userReport.boundaries.length > 0 && (
                <ReportSection title="Important system boundaries">
                  <div className="space-y-3">
                    {userReport.boundaries.map((item) => (
                      <div key={item.title} className="rounded-lg border border-white/10 bg-[#050b14] p-3">
                        <div className="text-sm font-semibold text-white">{item.title} - {item.status}</div>
                        <p className="mt-2 text-sm leading-6 text-slate-300">{item.explanation}</p>
                      </div>
                    ))}
                  </div>
              ...
              <details className="rounded-lg border border-white/10 bg-[#0a1122] p-4">
                <summary className="cursor-pointer text-sm font-semibold text-white">
                  Technical reasoning and evidence
                </summary>
                <div className="mt-3 space-y-3">
                  {userReport.technicalSections
